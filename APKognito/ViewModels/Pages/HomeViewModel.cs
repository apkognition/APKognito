...
public partial class HomeViewModel : LoggableObservableObject
{
    ...
    {
        RenamedPackageListing? package = await _dialogDispatcher.For<RenamedPackageSelector>().ShowAsync<RenamedPackageListing>();
    ...
    {
        App.NavigateTo<RenameConfigurationPage>();
    }
    ...
    private async Task StartPackageRenamingAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            foreach (string sourceApkPath in files)
            {
                ...
                PackageContext renameConfig = new()
                {
    ...
    private async Task<bool> VerifyToolInstallationAsync()
    {
        try
        {
            ...
            if (!File.Exists(apksignerJarPath))
            {
                ...
                if (!await WebGet.FetchAndDownloadGitHubReleaseAsync(Constants.APK_SIGNER_URL_LTST, apksignerJarPath, this, default, 1))
                {
                    allSuccess = false;
                }
            }
    ...
    private async Task UpdateFootprintInfoAsync()
    {
