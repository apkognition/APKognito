...
public static partial class WebGet
{
    ...
    {
        object? result = await FetchParseBaseTypeDocumentAsync(url, [["assets", num, "browser_download_url"]], logger, cToken);
    ...
    {
        return await FetchParseBaseTypeDocumentAsync(url, indexes, logger, cToken) as string?[] ?? [];
    }
    ...
    private static async Task<string?[]> FetchParseBaseTypeDocumentAsync(string url, object[][] indexes, IViewLogger? logger, CancellationToken cToken)
    {
        // if (!await VerifyConnectionAsync(logger))
        // {
        //     return [];
    ...
    {
        // This was added as an attempt to resolve this issue: https://github.com/apkognition/APKognito/issues/2
