...
public class ConfigurationFactory
{
    ...
    private static T? _Load_Bson<T>(string filePath, ConfigModifiers modifiers) where T : IKognitoConfig, new()
    {
        if (!File.Exists(filePath))
        {
            return new();
    ...
    private static T? Load_Bson<T>(string filePath, ConfigModifiers modifiers) where T : IKognitoConfig, new()
    {
        ...
        using (inputStream)
        using (BsonDataReader reader = new(inputStream))
        {
            var deserializer = new JsonSerializer();
            return deserializer.Deserialize<T>(reader);
        }
    ...
    private static void _Save_Bson<T>(T config, string filePath, ConfigModifiers modifiers) where T : IKognitoConfig
    {
    ...
    private static void Save_Bson<T>(T config, string filePath, ConfigModifiers modifiers) where T : IKognitoConfig
    {
        ...
        if (modifiers.HasFlag(ConfigModifiers.Compressed))
        {
            outputStream = new GZipStream(fileStream, CompressionLevel.Optimal);
        }
        ...
        using (outputStream)
        using (BsonDataWriter writer = new(outputStream))
        {
            serializer.Serialize(writer, config);
            writer.Flush();
        }
