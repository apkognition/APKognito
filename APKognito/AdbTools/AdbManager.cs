...
internal sealed class AdbManager : IDisposable
{
    ...
    public static async Task<AdbCommandOutput?> LoggedDeviceCommandAsync(string arguments, IViewLogger logger, string? deviceId = null, bool captureOutput = false, CancellationToken token = default)
    {
        ...
        await adbInstance.RunCommandAsync($"-s {deviceId} {arguments}",
            ...
            (sender, e) =>
            {
                ...
                if (captureOutput) stdOut!.AppendLine(e.Data);
                logger.Log(e.Data);
            },
            // StdErr
            (sender, e) =>
            {
                if (e.Data is null) return;
                if (captureOutput) stdErr!.AppendLine(e.Data);
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public static async Task<ICommandOutput> QuickGenericCommandAsync(string command, string arguments, bool noThrow = false, CancellationToken token = default)
    {
    ...
    public static async Task<AdbCommandOutput> InstallPackageAsync(string packagePath, bool noThrow = false, CancellationToken token = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(packagePath);
        DirectoryManager.FileExists(packagePath);
        return await QuickDeviceCommandAsync(@$"install -g ""{packagePath}""", noThrow: noThrow, token: token);
    }

    public static async Task<AdbCommandOutput> PushDirectoryAsync(string directoryPath, string remotePath, bool noThrow = false, CancellationToken token = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(directoryPath);
        ArgumentException.ThrowIfNullOrWhiteSpace(remotePath);
        DirectoryManager.DirectoryExists(directoryPath);
        return await QuickDeviceCommandAsync($"push \"{directoryPath}\" \"{remotePath}\"", noThrow: noThrow, token: token);
    }

    public static async Task<AdbCommandOutput> PushFileAsync(string filePath, string remotePath, bool noThrow = false, CancellationToken token = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(filePath);
        ArgumentException.ThrowIfNullOrWhiteSpace(remotePath);
        DirectoryManager.FileExists(filePath);
        return await QuickDeviceCommandAsync($"push \"{filePath}\" \"{remotePath}\"", noThrow: noThrow, token: token);
    }

    /// <summary>
    /// Checks if <paramref name="localPath"/> points to a directory or a file, and relays the given parameters to the correct handler.
    /// Throws if <paramref name="localPath"/> points to a non-existent item.
    /// </summary>
    ...
    public static async Task<AdbCommandOutput> PushFilesystemItemAsync(string localPath, string remotePath, bool noThrow = false, CancellationToken token = default)
    {
        ...
        {
            await PushDirectoryAsync(localPath, remotePath, noThrow, token);
        }
        else if (File.Exists(localPath))
        {
            await PushFileAsync(localPath, remotePath, noThrow, token);
        }

        throw new IOException($"No file or directory exists with the given path: {localPath}");
    }
